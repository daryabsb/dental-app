	...
	methods: {
		...
		async upload() {
			let that = this,
				...
				fmData = new FormData();
			fmData.append('image', data2blob(createImgUrl, mime), field + '.' + imgFormat);
			...
			// let imageResponse = await axios.put(url, fmData);
			let imageResponse = await this.$axios.put(url, fmData);

			this.$store.commit('PUSH_IMAGE', imageResponse.data);
			that.loading = 2;
			that.$emit('crop-upload-success', field, ki);
				...
				that.$emit('crop-upload-fail', field, ki);
			}
